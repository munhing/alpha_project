...
@section('content')
	...
	{{ Form::open(array('route' => 'reports', 'method' => 'PUT', 'files' => 'true', 'class' => 'form-horizontal', 'role' => 'form')) }}
		{{ Form::hidden('id', $report->id) }}
		{{ Form::hidden('filename', $report->filename) }}
		<div class="form-group">

			{{ Form::label('report_no', 'Report No:', ['class' =>'col-sm-2 control-label']) }}
			<div class="col-sm-6">
				{{ Form::text('report_no', $report->report_no, ['class' =>'form-control', 'placeholder' => 'Report No']) }}
		...
		<div class="form-group">
			...
			<div class="col-sm-6">
				{{ Form::select('type', $types, $report->type, ['class' =>'form-control', 'placeholder' => 'Type']) }}
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				...
				{{ Form::text('validity', $report->validity, ['class' =>'form-control', 'placeholder' => 'Validity']) }}
		...
		@if(!is_dir(public_path(). '/report_files/' . $report->filename ) && file_exists(public_path(). '/report_files/' . $report->filename ))
			<div class="form-group">
				<div class="col-sm-offset-2 col-sm-6">
					<a class="btn btn-info btn-circle btn-xl" href="{{ URL::asset('/report_files/' . $report->filename) }}" target="_blank"><i class="fa fa-file"></i></a>
				</div>
			</div>
		@endif
